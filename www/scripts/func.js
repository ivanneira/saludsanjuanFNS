...
//FUNCION PARA OBTENER UN CENTRO DE SALUD
function getCentroDeSaludDB_mapaNativo(id)
{
    ...
    db.executeSql('SELECT cs.*,dp.Nombre as Departamento, dp.Zona as Zona  FROM CentroDeSalud as cs inner join Departamentos as dp on dp.ID = cs.DepartamentoID where cs.ID='+id, [], function(rs) {
        ...
        if(rs.rows.item(0).Latitud != 0 || rs.rows.item(0).Longitud != 0) {
            ...
            xmap = plugin.google.maps.Map.getMap(document.getElementById("map_canvas"));

            // Wait until the map is ready status.
            // Initialize the map view

            xmap.addEventListener(plugin.google.maps.event.MAP_READY, function(){
                ...
                xmap.animateCamera({
                    ...
                    xmap.addMarker({
                        ...
                        title: "Según tu móvil te encuentras en este punto.",
                        snippet: "Salud San Juan",
